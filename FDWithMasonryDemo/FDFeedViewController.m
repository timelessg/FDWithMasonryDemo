...
@interface FDFeedViewController () <UIActionSheetDelegate,UITableViewDataSource,UITableViewDelegate>
...
- (void)viewDidLoad {
    ...
    self.listTabelView.frame = self.view.bounds;
    [self.view addSubview:self.listTabelView];
    [self.listTabelView registerClass:[FDFeedCell class] forCellReuseIdentifier:@"FDFeedCell"];
    
    self.listTabelView.fd_debugLogEnabled = YES;
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (editingStyle == UITableViewCellEditingStyleDelete) {
        ...
        [self.listTabelView deleteRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
    }
}
...
- (void)insertRow {
    if (self.feedEntitySections.count == 0) {
        ...
        [self.listTabelView insertRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
    }
}

- (void)insertSection {
    [self.feedEntitySections insertObject:@[self.randomEntity].mutableCopy atIndex:0];
    [self.listTabelView insertSections:[NSIndexSet indexSetWithIndex:0] withRowAnimation:UITableViewRowAnimationAutomatic];
}

- (void)deleteSection {
    if (self.feedEntitySections.count > 0) {
        [self.feedEntitySections removeObjectAtIndex:0];
        [self.listTabelView deleteSections:[NSIndexSet indexSetWithIndex:0] withRowAnimation:UITableViewRowAnimationAutomatic];
    }
}
#pragma mark - Getters
-(UITableView *)listTabelView{
    if (!_listTabelView) {
        _listTabelView = [[UITableView alloc] initWithFrame:CGRectZero style:UITableViewStylePlain];
